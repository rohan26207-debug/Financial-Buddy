...
import { Download, Upload, RefreshCw, Globe, Info, Wallet, ChevronRight, Lock, ShieldAlert, Mail, BellRing } from 'lucide-react';
...
export default function Settings() {
  const { state, updateSettings, exportData, importData, resetData, markBackedUp } = useStore();
  ...
  const onEmailBackup = () => {
    ...
    if (!email) {
      toast.error('Enter a backup email below first');
      return;
    }
    try {
      // Always download the file so the user can attach it.
      triggerDownload();
      markBackedUp();
      ...
      toast.success('Backup downloaded — attach it in your email');
    } catch (e) {
